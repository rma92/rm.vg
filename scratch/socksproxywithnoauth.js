...
//Source:https://gist.github.com/telamon/1127459
var States = {
  CONNECTED:0,
  VERIFYING:1,
  READY:2,
...
function accept(socket){
  ...
  var handshake = function(chunk){
    ...
    if(socket.methods.indexOf(AuthMethods.USERPASS)){
      ...
      socket.write(resp);
    }else if(socket.methods.indexOf(AuthMethods.GSSAPI))
    {
      console.log("$$$");
      ...
      socket.write( resp );
    }else if(socket.methods.indexOf(AuthMethods.NOAUTH))
    {
      console.log("c1");
      socket.pstate=States.READY;
      socket.handleRequest = handleRequest.bind( socket );
      ...
      console.log("bad authentication method");
      resp[1]=0xFF
      socket.end(resp);
    }
  }
...
function authUSERPASS(chunk){
 ...
 resp[1]=0xff;
 if(chunk[0] != 1){
  console.log("wrong auth version, closing connection");
...
function handleRequest(chunk){
  ...
  console.log('AddressR ', address, '$ ', offset);
  var port = chunk.readUInt16BE(offset,'big');
